...
| `AuditStore` | port in `domain/publish`, impl in `outbound/persistence` | One `INSERT` into `audit` per published message, **before** the ack (RF-29). Skip republish of already-present movements (`UNIQUE (txn_dedup, published_at)` constraint, generated column). | `audit` |

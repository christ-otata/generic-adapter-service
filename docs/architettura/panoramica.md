...
| Audit store | Spring Data JDBC | One record per successfully published message, with origin `topic/partition/offset`, business keys and `processing_id`, written **before** the offset commit. `UNIQUE (txn_dedup, published_at)` on the generated column `txn_dedup` for movement skip-republish. | RF-29, RNF-14 |

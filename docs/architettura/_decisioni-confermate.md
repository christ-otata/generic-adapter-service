...
- **AD-retry-mechanism** → *Spring Kafka `@RetryableTopic`* for E3/E7 (retry topics + backoff + dispatch to a case record on retry exhaustion; custom `RetryTopicConfiguration` for per-category routing).
